...
# ─── Load Secrets ─────────────────────────────────────────────────────────────
# Make sure in your Streamlit Cloud UI, under "Secrets", you have:
#
...
try:
    ...
    missing = e.args[0]
    st.error(f"Secret `{missing}` not found. Please add it under Settings → Secrets.")
    st.stop()
...
def generate_text(prompt: str) -> str:
    ...
    payload = {
        "model": "gpt-4o",  # swap for your desired OpenRouter‐served model
        "messages": [
            {
                "role": "system",
                "content": (
    ...
    data = resp.json()
    # some models return data["choices"], others data["results"] → adjust if needed
    return data["choices"][0]["message"]["content"]

# ─── Image Generation via Replicate ───────────────────────────────────────────
def generate_image(prompt: str) -> bytes:
    """
    Calls lucataco/realistic-vision-v5.1 on Replicate for NSFW image generation.
    """
    model_ref = (
