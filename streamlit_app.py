...
with st.sidebar:
    ...
    scheduler = st.selectbox(
        "Scheduler:", ["DPMSolverMultistep", "PNDM", "DDIM"], index=0
    )

    neg_extra = st.text_area(
        "Extra Negative Terms (optional):", value="", height=80
    ).strip()
    negative_prompt = (
...
full_prompt = (
    f"Perform explicitly: {action}. {prompt_base}" if action else prompt_base
)
...
if st.button("Generate"):
    ...
    try:
        ...
        # Generate video via text2video
        st.success("Image done. Generating video...")
        with st.spinner("Generating video (text-to-video)..."):
            video_out = run_with_retry(
                "wan-video/wan-2.1-1.3b", {"prompt": full_prompt}
            )
        ...
        elif isinstance(video_out, str) and video_out.startswith("http"):
            st.video(video_out)
        else:
